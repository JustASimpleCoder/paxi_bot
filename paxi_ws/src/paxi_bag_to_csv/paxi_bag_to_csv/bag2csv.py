...
class MetadataYaml():
    ...
    def get_topic_metadata(self):
        with open(self.metada_file_path, 'r') as file:
            ...
            for topic_data_with_msg_count in data['rosbag2_bagfile_information']['topics_with_message_count']:
                self.topic_data.append( (topic_data_with_msg_count["topic_metadata"]['name'], topic_data_with_msg_count["topic_metadata"]['type'])  )
...
class bag_to_csv():
    def __init__(self, bag_folder_path : str, file_uri : str, out_csv_folder_path : Optional[str]):
        ...
        self.csv_path = self.bag_info.folder_path if out_csv_folder_path is None else out_csv_folder_path
    ...
    def generate_csv_for_a_topic(self, topic_and_topic_msg_type : Tuple[str , str]):
        ...
        with open(self.csv_path + "_" + topic.replace("/","_") + ".csv", mode ='w', newline='') as file:
...
def main():
    ...
    bag_2_csv = bag_to_csv(bag_folder, file_uri, "/home/j/robotics/paxi_bot_dev/paxi_bot/paxi_ws/bag_data_csv/")
