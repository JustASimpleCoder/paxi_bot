...
#
from linear_regression import CSVLinRegression as lin_reg
...
class LinRegNode(Node):
    ...
    def log_line_reg_data_right(self, model_type : str):
        ...
        self.get_logger().info(f"R-squared Left {model_type} [{self.l_wheel_lin_reg.get_score()[idx]}]")
        self.get_logger().info(f"Intercept Left {model_type} [{self.l_wheel_lin_reg.get_y_intercept()[idx]}]")
        self.get_logger().info(f"Slope Left {model_type}     [{self.l_wheel_lin_reg.get_coefficient()[idx]}]")
    ...
    def log_line_reg_data_left(self, model_type : str):
        ...
        if model_type == 'neg':
            idx = 1
        print(f"**** LEFT WHEEL DATA {model_type.upper()} ****")
        self.get_logger().info(f"R-squared Left {model_type} [{self.r_wheel_lin_reg.get_score()[idx]}]")
        self.get_logger().info(f"Intercept Left {model_type} [{self.r_wheel_lin_reg.get_y_intercept()[idx]}]")
        self.get_logger().info(f"Slope Left {model_type}     [{self.r_wheel_lin_reg.get_coefficient()[idx]}]")
