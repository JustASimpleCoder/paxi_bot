...
public:
  ...
  void publish_data(
    const std::shared_ptr<typename rclcpp::Publisher<MsgT>> & pub, const MsgT & msg) const
  {
    pub->publish(msg);
  }
  ...
  void publish_real_time(
    const SerialFeedback & feedback, bool connected, const sensor_msgs::msg::Imu & imu_msg,
