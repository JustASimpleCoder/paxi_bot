...
void ImuProcessing::update_imu(const rclcpp::Time time, const SerialFeedback & feedback)
{
  ...
  imu_msg_.angular_velocity.x =
    static_cast<double>(feedback.gyro_x) / GYRO_TO_DEG_S * (M_PI / 180.0);
  imu_msg_.angular_velocity.y =
    static_cast<double>(feedback.gyro_y) / GYRO_TO_DEG_S * (M_PI / 180.0);
  imu_msg_.angular_velocity.z =
    static_cast<double>(feedback.gyro_z) / GYRO_TO_DEG_S * (M_PI / 180.0);
  ...
  quat_32_w =
    (static_cast<int32_t>(feedback.quat_w_high) << 16) | static_cast<int32_t>(feedback.quat_w_low);
  quat_32_x =
    (static_cast<int32_t>(feedback.quat_x_high) << 16) | static_cast<int32_t>(feedback.quat_x_low);
  quat_32_y =
    (static_cast<int32_t>(feedback.quat_y_high) << 16) | static_cast<int32_t>(feedback.quat_y_low);
  quat_32_z =
    (static_cast<int32_t>(feedback.quat_z_high) << 16) | static_cast<int32_t>(feedback.quat_z_low);
