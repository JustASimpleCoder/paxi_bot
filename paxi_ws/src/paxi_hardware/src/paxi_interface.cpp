...
bool PaxiInterface::get_params_from_xacro(const hardware_interface::HardwareInfo & hardware_info)
{
  ...
  try {
    ...
    validate_params &=
      encoder_.set_wheel_radius(std::stod(hardware_info.hardware_parameters.at("wheel_radius")));
    validate_params &= encoder_.set_wheel_separation(
      std::stod(hardware_info.hardware_parameters.at("wheel_separation")));
    validate_params &=
      encoder_.set_max_velocity(std::stod(hardware_info.hardware_parameters.at("max_velocity")));
...
bool PaxiInterface::check_joints_and_state(const hardware_interface::HardwareInfo & hardware_info)
{
  for (const hardware_interface::ComponentInfo & joint : hardware_info.joints) {
    ...
    if (joint.state_interfaces.size() != 2) {
      RCLCPP_FATAL(
        rclcpp::get_logger(LOGGER_HARDWARE), "Joint '%s' has %zu state interface. 2 expected.",
  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  if (!serial_port_.is_open()) {
    RCLCPP_ERROR(
      rclcpp::get_logger(LOGGER_HARDWARE), "Serial port [%s] to hoverboard is closed",
      serial_port_.get_port_name().c_str());
  ...
  if (!serial_port_.is_connected()) {
    RCLCPP_ERROR_THROTTLE(
      rclcpp::get_logger(LOGGER_HARDWARE), *paxi_interface_node_->get_clock(), 5000,
