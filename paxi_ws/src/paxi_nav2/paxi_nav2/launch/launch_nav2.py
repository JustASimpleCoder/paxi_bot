...
def generate_launch_description():
    ...
    nav2_params_file = PathJoinSubstitution(
        [pkg_paxi_navigation, "config", "nav2_params.yaml"]
    )

    map_file = PathJoinSubstitution([pkg_paxi_navigation, "maps", "map.yaml"])

    robot_urdf = PathJoinSubstitution(
        [pkg_paxi_description, "urdf", "paxi_bot.urdf.xacro"]
    )

    rviz_config_file = PathJoinSubstitution(
        [pkg_paxi_navigation, "rviz", "nav2_config.rviz"]
    )
